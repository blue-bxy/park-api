...
@section('content')
    ...
    <div class="float">
        <div class="float_flex">
            ...
            <!-- 日期类型 -->
            <div class="Order">
                <p style="font-size: 20px;margin-left: 4%;">修改账户</p></td>
            </div>
            <!-- 表格 -->
                ...
                <div class="instrtion">
                    <div class="instrtion_king">
                            ...
                            <div>
                                <select name="property_id" lay-verify="required">
                                    @foreach($properties as $name=>$id)
                                        <option value="{{$id}}" @if($data->property_id==$id) selected @endif>{{$name}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="instrtion_input">
                            ...
                            <input type="text" name="ein" id="" class="layui-input" value="{{$data->ein}}" lay-verify="required">
                        </div>
                        <div class="instrtion_input">
                            <div class="instrtion_span">
                                <span>信用代码：</span>
                            </div>
                            <input type="text" name="credit_code" id="" class="layui-input" value="{{$data->credit_code}}" lay-verify="required">
                            </div>
                            <div class="instrtion_input">
                                <div class="instrtion_span">
                                <span>银行账户：</span>
                            </div>
                            <input type="text" name="bank_account" id="" class="layui-input" value="{{$data->bank_account}}" lay-verify="required">
                        </div>
                        <div class="instrtion_input">
                            <div class="instrtion_span">
                                <span>银行名称：</span>
                            </div>
                            <input type="text" name="bank_name" id="" class="layui-input" value="{{$data->bank_name}}" lay-verify="required">
                        </div>
                        <div class="instrtion_input">
                            <div class="instrtion_span">
                                <span>地址：</span>
                            </div>
                            <input type="text" name="address" id="" class="layui-input" value="{{$data->address}}" lay-verify="required">
                        </div>
                        <div class="instrtion_input">
                            <div class="instrtion_span">
                                <span>联系电话：</span>
                            </div>
                            <input type="text" name="phone" id="" class="layui-input" value="{{$data->phone}}" lay-verify="phone">
                        </div>
                        <div class="Order_input" style="margin-left:44%;height: 35px;margin-bottom: 15px;">
                            <button type="button" class="layui-btn" lay-submit="" lay-filter="update">确定</button>
                ...
                $.ajax({
                    ...
                    success: function (res) {
                        if (res.code == 1) {
                            ...
                            $(".layui-btn").removeClass('layui-disabled').removeAttr('disabled');
                            layer.msg('修改失败', {icon: 2, time: 1500, shade: 0.1});
                            return false;
                        }
                    }
                })
